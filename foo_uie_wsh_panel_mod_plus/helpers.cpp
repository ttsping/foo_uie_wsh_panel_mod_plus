...
namespace helpers
{
	...
	HRESULT get_album_art(BSTR rawpath, IGdiBitmap ** pp, int art_id, VARIANT_BOOL need_stub)
	{
		...
		album_art_extractor_instance_ptr aaei;
		...
		try
		{
			aaei = album_art_extractor::g_open(NULL,pfc::stringcvt::string_utf8_from_wide(rawpath),abort);
			
			data = aaei->query(what, abort);
		...
		catch (std::exception &)
		{
			if (need_stub)
			{
				album_art_extractor_instance_v2::ptr aaeiv2_stub = static_api_ptr_t<album_art_manager_v2>()->open_stub(abort);

				try 
				{
					album_art_data_ptr data = aaeiv2_stub->query(what, abort);

					ret = query_album_art(aaeiv2_stub, what, false, NULL);
				} catch (std::exception &) {}
			}
		}
	...
	HRESULT get_album_art_v3( const metadb_handle_ptr & handle, IGdiBitmap ** pp, BSTR pattern , VARIANT_BOOL load_embed, VARIANT_BOOL use_fallback , int art_id, VARIANT_BOOL need_stub, VARIANT_BOOL no_load /*= VARIANT_FALSE*/, pfc::string_base * image_path_ptr /*= NULL*/ )
	{
		if(handle.is_empty())return E_INVALIDARG;
		if(!pp)return E_POINTER;
		...
		GUID what;
		abort_callback_dummy abort;
		static_api_ptr_t<album_art_manager_v3> aamv3;
		album_art_extractor_instance_v2::ptr aaeiv2;
		IGdiBitmap * ret = NULL;

		what = helpers::convert_artid_to_guid(art_id);

		my_album_art_manager_config::ptr cfg = new service_impl_t<helpers::my_album_art_manager_config>(
			pfc::stringcvt::string_utf8_from_wide(pattern),
			what,
			!!load_embed,
			!!use_fallback);
		...
		try
		{
			aaeiv2 = aamv3->open_v3(
				pfc::list_single_ref_t<metadb_handle_ptr>(handle), 
				pfc::list_single_ref_t<GUID>(what), 
				cfg,abort);
		...
		catch (std::exception &)
		{
			if (need_stub)
			{
				album_art_extractor_instance_v2::ptr aaeiv2_stub = aamv3->open_stub(abort);
	...
	HRESULT get_remote_text( const service_ptr_t<http_request> client, const char* url, pfc::string_base& p_out)
	{
		...
		if (client.is_empty())return S_FALSE;
		
		file::ptr response;
		abort_callback_dummy abort;
		try
		{
			response = client->run(url,abort);
			response->read_string_raw(p_out,abort);
		}
		catch (...){
			p_out.reset();
			return S_FALSE;
	...
	void get_remote_text_async::run()
	{
		TRACK_FUNCTION();

		if (m_client.is_empty())return;

		pfc::stringcvt::string_utf8_from_wide url8((const wchar_t*)m_url);
		pfc::string8_fast temp;
		file::ptr response;
		abort_callback_dummy abort;
		try{
			response = m_client->run(url8,abort);
			response->read_string_raw(temp,abort);
		}
		catch(...){
		...
		t_param param(reinterpret_cast<unsigned>(this),temp);
