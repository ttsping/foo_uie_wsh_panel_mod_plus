...
public:
	...
	STDMETHODIMP FillPie(float x,float y,float w, float h, float start_angle, float sweep_angle, VARIANT color);
	...
	STDMETHODIMP DrawPie(float x,float y,float w, float h, float start_angle, float sweep_angle, float line_width,VARIANT color);
...
public:
	...
	STDMETHODIMP AddFontResEx(BSTR path, VARIANT_BOOL fl,UINT * p);
	STDMETHODIMP RemoveFontResEx(BSTR path, VARIANT_BOOL fl, UINT * p);
	STDMETHODIMP CreatePrivateFontCollection(IPrivateFontCollection** pp);
	STDMETHODIMP CloneGraphics(IGdiGraphics *g,INT x,INT y, INT w,INT h,IGdiBitmap** pp);
};
...
public:
    STDMETHODIMP get__ptr(void ** pp);
	STDMETHODIMP Eval(VARIANT_BOOL force, VARIANT_BOOL force_read , BSTR * pp );
	STDMETHODIMP EvalWithMetadb(IFbMetadbHandle * handle, VARIANT_BOOL force , BSTR * pp);
};
...
public:
	...
	STDMETHODIMP ShowPreferences(BSTR guid_str);
	STDMETHODIMP ClearPlaylist();
	STDMETHODIMP LoadPlaylist();
	STDMETHODIMP LoadPlaylistV2(BSTR path , VARIANT_BOOL *p);
	STDMETHODIMP SavePlaylist();
	STDMETHODIMP SavePlaylistV2(BSTR path , IFbMetadbHandleList * handles , VARIANT_BOOL overwrite ,VARIANT_BOOL *p);
...
public:
	...
	STDMETHODIMP GetAlbumArtV3(IFbMetadbHandle * handle, BSTR pattern ,int art_id, VARIANT_BOOL need_stub, VARIANT_BOOL load_embed, VARIANT_BOOL use_fallback ,IGdiBitmap **pp);
	...
	STDMETHODIMP IsAeroEnabled(VARIANT_BOOL * p);
	STDMETHODIMP DecodeBase64Image(BSTR str,IGdiBitmap** pp);
	STDMETHODIMP GetClipboardText(BSTR* pp);
	STDMETHODIMP SetClipboardText(BSTR str);
	STDMETHODIMP PlaySound(BSTR path,INT flag);
	STDMETHODIMP LoadCursorX(BSTR path,INT* p);
	STDMETHODIMP SetCursorX(INT id,INT* p);
	STDMETHODIMP ReleaseCursorX(INT id);
	STDMETHODIMP IsVistaOrGreater(VARIANT_BOOL* p);
	STDMETHODIMP LCMapString(BSTR str,INT lcid,INT flag,BSTR* pp);
...
public:
	...
	STDMETHODIMP DrawThemeTextEx(IGdiGraphics * gr, BSTR text, int color, int x, int y, int w, int h, int glow_size,int format);
};
...
public:
	...
	STDMETHODIMP CreateGlass(UINT l,UINT t,UINT r,UINT b,UINT mode);
	STDMETHODIMP SetWindowTransparency(UINT opacity, int color);
	STDMETHODIMP MsgBox(BSTR caption,BSTR prompt,UINT type,int* p);
	STDMETHODIMP InputBox(BSTR caption,BSTR prompt,BSTR defval,VARIANT_BOOL only_num,BSTR* pp);
	STDMETHODIMP FileDialog(INT mode,BSTR title,BSTR filetype,BSTR defext,BSTR* pp);
	STDMETHODIMP PrintWindow(VARIANT_BOOL client_only,IGdiBitmap** pp);
...
class PrivateFontCollectionObj : public IDispatchImpl3<IPrivateFontCollection>
{
protected:
...
public:
	...
	STDMETHODIMP AddFont(BSTR path,INT* p);
...
class HttpRequest : public IDisposableImpl4<IHttpRequest>
{
	...
	http_request::ptr m_client;

public:
	HttpRequest(const char* type)
	{
		try
		{
			m_client = static_api_ptr_t<http_client>()->create_request(type);
		}
...
public:
	STDMETHODIMP AddHeader(BSTR line);
	STDMETHODIMP AddPostData(BSTR name,BSTR value);
	STDMETHODIMP Run(BSTR url,BSTR* pp);
