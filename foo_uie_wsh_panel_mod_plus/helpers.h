...
namespace helpers
{
	...
	class my_album_art_manager_config : public album_art_manager_config
	{
	public:

		my_album_art_manager_config(const char* p_pattern , const GUID& p_guid , bool p_embeded , bool p_fallback)
			: m_pattern(p_pattern)
			, m_type(p_guid)
			, m_use_embeded(p_embeded)
			, m_use_fallbacks(p_fallback){}

		virtual bool get_external_pattern(pfc::string_base & out, const GUID & type){
			if (!pfc::guid_compare(type,m_type)){
				out = m_pattern;
				return true;
			}
			return false;
		}
		virtual bool use_embedded_pictures() { return m_use_embeded; };
		virtual bool use_fallbacks() { return m_use_fallbacks; };

	private:
		pfc::string8 m_pattern;
		bool m_use_embeded;
		bool m_use_fallbacks;
		GUID m_type;
	};


	class get_remote_text_async : public simple_thread_task
	{
	public:
		struct t_param
		{
			unsigned cookie;
			pfc::stringcvt::string_wide_from_utf8 text;

			t_param(unsigned p_cookie, const char* p_text):cookie(p_cookie),text(p_text){
			}

	
		};

		get_remote_text_async(service_ptr_t<http_request> client, HWND notify_wnd,BSTR url):
		m_client(client),m_notify_hwnd(notify_wnd),m_url(url)
		{}
	...
	private:
		HWND m_notify_hwnd;
		_bstr_t m_url;
		service_ptr_t<http_request> m_client;
	};
