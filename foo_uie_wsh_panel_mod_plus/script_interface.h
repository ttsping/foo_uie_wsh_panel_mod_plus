...
__interface IGdiGraphics: IGdiObj
{
	...
	STDMETHOD(FillPie)(float x,float y,float w, float h, float start_angle, float sweep_angle, VARIANT color);
	...
	STDMETHOD(DrawPie)(float x,float y,float w, float h, float start_angle, float sweep_angle, float line_width,VARIANT color);
...
__interface IGdiUtils: IDispatch
{
	...
	STDMETHOD(AddFontResEx)(BSTR path,[defaultvalue(0)] VARIANT_BOOL fl,[out,retval] UINT * p);
	STDMETHOD(RemoveFontResEx)(BSTR path,[defaultvalue(0)] VARIANT_BOOL fl,[out,retval] UINT * p);
	STDMETHOD(CreatePrivateFontCollection)([out,retval]__interface IPrivateFontCollection** pp);
	STDMETHOD(CloneGraphics)(IGdiGraphics *g,INT x,INT y, INT w,INT h,[out,retval]IGdiBitmap** pp);
};
...
__interface IFbMetadbHandle: IDisposable
{
	...
	STDMETHOD(GetFileInfo)([defaultvalue(0)] VARIANT_BOOL force,[out,retval] IFbFileInfo ** pp);
	STDMETHOD(UpdateFileInfo)(IFbFileInfo * fileinfo, [defaultvalue(0)] VARIANT_BOOL force);
...
{
    [propget] STDMETHOD(_ptr)([out,retval] void ** pp);
	STDMETHOD(Eval)([defaultvalue(0)] VARIANT_BOOL force, [defaultvalue(0)] VARIANT_BOOL force_read,[out,retval] BSTR* pp);
	STDMETHOD(EvalWithMetadb)(IFbMetadbHandle * handle, [defaultvalue(0)] VARIANT_BOOL force , [out,retval] BSTR * pp);
};
...
__interface IFbUtils: IDispatch
{
	...
	STDMETHOD(ShowPreferences)([defaultvalue("")] BSTR guid_str);
	STDMETHOD(ClearPlaylist)();
	STDMETHOD(LoadPlaylist)();
	STDMETHOD(LoadPlaylistV2)(BSTR path , [out,retval] VARIANT_BOOL *p);
	STDMETHOD(SavePlaylist)();
	STDMETHOD(SavePlaylistV2)(BSTR path , IFbMetadbHandleList * handles , [defaultvalue(0)] VARIANT_BOOL overwrite , [out,retval] VARIANT_BOOL *p);
...
__interface IThemeManager: IDisposable
{
	...
	// Vista+
	STDMETHOD(DrawThemeTextEx)(IGdiGraphics * gr, BSTR text, int color,int x, int y, int w, int h,[defaultvalue(5)] int glow_size, [defaultvalue(0)] int format);
};
...
__interface IWSHUtils: IDispatch
{
	...
	STDMETHOD(GetAlbumArtV3)(IFbMetadbHandle * handle, BSTR pattern ,[defaultvalue(0)] int art_id, [defaultvalue(-1)] VARIANT_BOOL need_stub,  [defaultvalue(-1)] VARIANT_BOOL load_embed,[defaultvalue(-1)] VARIANT_BOOL use_fallback , [out,retval] IGdiBitmap **pp);
	...
	STDMETHOD(IsAeroEnabled)([out,retval] VARIANT_BOOL *p);
	STDMETHOD(DecodeBase64Image)(BSTR str,[out,retval] IGdiBitmap** pp);
	STDMETHOD(GetClipboardText)([out,retval]BSTR* pp);
	STDMETHOD(SetClipboardText)(BSTR str);
	STDMETHOD(PlaySound)(BSTR path,INT flag);
	STDMETHOD(LoadCursorX)(BSTR path,[out,retval]INT* p);
	STDMETHOD(SetCursorX)(INT id,[out,retval]INT* p);
	STDMETHOD(ReleaseCursorX)(INT id);
	STDMETHOD(IsVistaOrGreater)([out,retval]VARIANT_BOOL* p);
	STDMETHOD(LCMapString)(BSTR str,INT lcid,INT flag,[out,retval]BSTR* pp);
...
__interface IWindow
{
	...
	STDMETHOD(CreateGlass)(UINT l,UINT t,UINT r,UINT b,[defaultvalue(0)] UINT mode);
	STDMETHOD(SetWindowTransparency)(UINT opacity, int color);
	STDMETHOD(MsgBox)(BSTR caption,BSTR prompt,UINT type,[out,retval]int* p);
	STDMETHOD(InputBox)([defaultvalue("")]BSTR caption,[defaultvalue("")]BSTR prompt,[defaultvalue("")] BSTR defval,[defaultvalue(0)]VARIANT_BOOL only_num,[out,retval]BSTR *pp);
	STDMETHOD(FileDialog)([defaultvalue(0)]INT mode,[defaultvalue("Open")]BSTR title,[defaultvalue("All Files|*.*")]BSTR filetype,[defaultvalue("*")]BSTR defext,[out,retval]BSTR* pp);
	STDMETHOD(PrintWindow)([defaultvalue(-1)] VARIANT_BOOL client_only,[out,retval]IGdiBitmap** pp);
...
__interface IPrivateFontCollection
{
	[propget] STDMETHOD(FamilyCount)([out,retval]INT* p);

	STDMETHOD(AddFont)(BSTR path,[out,retval] INT* p);
	STDMETHOD(GetFont)(BSTR name,float size,[range(Gdiplus::FontStyleRegular, Gdiplus::FontStyleStrikeout)]int style,[out,retval] IGdiFont** p);
};
