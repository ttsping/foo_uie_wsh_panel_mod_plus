...
STDMETHODIMP FbPlaylistMangerTemplate::GetPlaylistAutoName( UINT playlistIndex, BSTR* name )
{
	TRACK_FUNCTION();
	...
	pfc::string8_fast temp,temp2;
	static_api_ptr_t<playlist_manager>()->playlist_get_name(playlistIndex,temp2);
	if(static_api_ptr_t<playlist_incoming_item_filter_v3>()->auto_playlist_name(items,temp)){
		*name = SysAllocString(pfc::stringcvt::string_wide_from_utf8(temp));
	}
	else{
		*name = SysAllocString(pfc::stringcvt::string_wide_from_utf8(temp2));
	}
...
STDMETHODIMP FbPlaylistMangerTemplate::PlaylistLock( UINT playlistIndex, UINT flag )
{
	TRACK_FUNCTION();

	service_impl_t<wsh_playlist_lock> * p_lock = new service_impl_t<wsh_playlist_lock>(flag);

	if(static_api_ptr_t<playlist_manager>()->playlist_lock_install(playlistIndex,p_lock)){
		wsh_playlist_lock::add_playlist_locker(playlistIndex,p_lock);
	}

	return S_OK;
}

STDMETHODIMP FbPlaylistMangerTemplate::PlaylistUnLock( UINT playlistIndex )
{
	TRACK_FUNCTION();
	wsh_playlist_lock::ptr p_lock;

	if (wsh_playlist_lock::get_playlist_locker(playlistIndex,p_lock)){
		static_api_ptr_t<playlist_manager>()->playlist_lock_uninstall(playlistIndex,p_lock);
	}

	return S_OK;
}

STDMETHODIMP FbPlaylistMangerTemplate::IsPlaylistLocked( UINT playlistIndex , VARIANT_BOOL * outIsLocked )
{
	TRACK_FUNCTION();

	if(!outIsLocked)return E_POINTER;

	(*outIsLocked) = TO_VARIANT_BOOL(static_api_ptr_t<playlist_manager>()->playlist_lock_is_present(playlistIndex));
...
STDMETHODIMP FbPlaylistManager::GetPlaylistAutoName( UINT playlistIndex, BSTR* name )
{
	return FbPlaylistMangerTemplate::GetPlaylistAutoName(playlistIndex,name);
}

STDMETHODIMP FbPlaylistManager::ProcessLocationsAsync( VARIANT urls )
{
	return FbPlaylistMangerTemplate::ProcessLocationsAsync(urls);
}

STDMETHODIMP FbPlaylistManager::PlaylistLock( UINT playlistIndex, UINT flag )
{
	return FbPlaylistMangerTemplate::PlaylistLock(playlistIndex,flag);
}

STDMETHODIMP FbPlaylistManager::PlaylistUnLock( UINT playlistIndex )
{
	return FbPlaylistMangerTemplate::PlaylistUnLock(playlistIndex);
}
