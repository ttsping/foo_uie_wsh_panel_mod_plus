...
void wsh_panel_window::on_http_run_done( LPARAM lp )
{
	...
	using namespace helpers;
	get_remote_text_async::t_param* param = reinterpret_cast<get_remote_text_async::t_param*>(lp);
	VARIANTARG args[2];


	args[0].vt = VT_BSTR;
	args[0].bstrVal = SysAllocString(param->text.get_ptr());
	args[1].vt = VT_I4;
	args[1].lVal = param->cookie;
	script_invoke_v(CallbackIds::on_http_run_done, args, _countof(args));
}

void wsh_panel_window::on_process_locations_done( WPARAM wp )
{
	TRACK_FUNCTION();

	simple_callback_data_scope_releaser< simple_callback_data< metadb_handle_list > > data(wp);
	VARIANTARG args[1];
	FbMetadbHandleList * handles = new com_object_impl_t<FbMetadbHandleList>(data->m_item);

	args[0].vt = VT_DISPATCH;
	args[0].pdispVal = handles;
	script_invoke_v(CallbackIds::on_process_locations_done, args, _countof(args));
